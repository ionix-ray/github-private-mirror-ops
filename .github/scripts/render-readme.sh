...
L.append("## Workflows")
L.append("")
L.append("- **New Private Fork** — `Actions → New Private Fork` (manual, input-driven). Opens a registration PR that adds one intent file.")
...
L.append("- Secrets: per-owner classic PATs stored as `GH_SYNC_PAT_<OWNER>`. Never accepted via dispatch inputs.")
