...
echo "== is_sha =="
check "full sha" "yes" "$(is_sha d21c0d08ef9500901fda7c4deec1c7cacf0781cc; echo yes)"
check "short sha" "yes" "$(is_sha abc1234; echo yes)"
check "non-hex rejected" "no" "$(is_sha 'xyz'; echo no)"
check "ref rejected" "no" "$(is_sha 'refs/heads/main'; echo no)"
check "injection rejected" "no" "$(is_sha 'abc;rm -rf /'; echo no)"
check "empty rejected" "no" "$(is_sha ''; echo no)"
